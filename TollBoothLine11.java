...
public class TollBoothLine 
{
	...
	public int insert(Vehicle insVeh)
	{
		...
		if(!isFull[lowestIndex])// check if line with the least amount of vehicles has less then 25 cars
		{
			...
			if(numItems[lowestIndex] > tempMax)
			{
				maxLen[lowestIndex] = numItems[lowestIndex];
			}
	...
	// get max length and getVehicleInALine added for the main
	public int getMaxLength(int whichLine)
	{
		return maxLen[whichline];
	}
	
	public Vehicle getVehicleInALine(int whichline, int whatPosition)
	{
